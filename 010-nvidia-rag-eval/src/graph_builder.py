...
def graph_rag_answer(query: str, graph: nx.DiGraph, model: str = "gpt-5.4-mini", client=client) -> dict:
...
if __name__ == "__main__":
    ...
    # --- DEBUG: see exactly which nodes matched and their degree ---
    query_entities_result = extract_entities_relationships(query)
    query_entities = query_entities_result.get("entities", [])
    print(f"Query entities: {query_entities}")

    for entity in query_entities:
        for candidate in find_matching_nodes(entity, graph):
            print(f"  '{entity}' matched node '{candidate}' with degree {graph.degree(candidate)}")
    # --- end debug ---
